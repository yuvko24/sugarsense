...
// Fetching doctor's personal details
...
// Retrieving patients
$patients = [];
$stmt = $conn->prepare("SELECT u.id, u.id_number, u.full_name, u.profile_picture FROM doctor_patient dp JOIN users u ON dp.patient_id = u.id WHERE dp.doctor_id = ?");
$stmt->bind_param("i", $doctorId);
$stmt->execute();
$result = $stmt->get_result();
...
<head>
  ...
  <title>בחירת מטופלת - רופאה</title>
...
<header>
 ...
 <main class="container py-5">
  <div class="text-center mb-4">
    <img src="<?= htmlspecialchars($doctorImage); ?>" alt="תמונת רופאה" width="100" height="100" class="rounded-circle border border-2 border-secondary shadow-sm mb-2">
    <h2>שלום, <?= htmlspecialchars($doctorName); ?>! 🩺</h2>
  </div>
  <h4 class="text-center mb-4">בחרי את המטופלת שלך</h4>
  <div class="text-center mb-5">
    <input type="text" id="patientSearch" class="form-control w-50 mx-auto" placeholder="🔍 חיפוש לפי שם מטופלת...">
  </div>
  <div class="row justify-content-center">
    <p id="noResultsMessage" class="text-center text-muted mt-4" style="display: none;">
      לא נמצאו מטופלות בשם שחיפשת 😕
    </p>
    ...
    <?php foreach ($patients as $patient): ?>
      <div class="col-md-4 mb-4 patient-card">
        <div class="card shadow-sm text-center p-3">
          ...
          <a href="dashboard_doctor.php?patient_id=<?= $patient['id'] ?>" class="btn text-white" style="background-color: #f4b6bd;">כניסה לפרופיל</a>
        </div>
      </div>
    <?php endforeach; ?>
  </div>
