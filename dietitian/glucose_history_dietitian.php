...
$query = "SELECT id, reading_date, time_slot, glucose_level, is_normal, dietitian_note, doctor_note FROM glucose_readings WHERE patient_id = ?";
$params = [$patientId];
$types = "i";
...
<head>
  ...
  <title>ניטורי סוכר - דיאטנית</title>
...
<main class="container py-5 flex-grow-1">
  <div class="position-relative text-center mb-4">
    <a href="menu_dietitian.php?type=glucose" class="btn btn-outline-secondary btn-sm position-absolute top-0 start-0 mt-1 ms-2">🔙 חזרה</a>
      ...
      <div class="card mb-4 shadow-sm">
        ...
        <div class="card-body">
          <?php if (!empty($reading['dietitian_note'])): ?>
            <div class="alert alert-info persistent">
              🗨️ <strong>ההערה שלי:</strong><br>
              <?= nl2br(htmlspecialchars($reading['dietitian_note'])); ?>
              <div class="mt-2 d-flex justify-content-end gap-2">
                <a href="edit_glucose_comment.php?reading_id=<?= $reading['id'] ?>" class="btn btn-sm btn-outline-primary">✏️ ערכי</a>
                <a href="delete_glucose_comment.php?reading_id=<?= $reading['id'] ?>" class="btn btn-sm btn-outline-danger" onclick="return confirm('האם את בטוחה שברצונך למחוק את ההערה?');">🗑️ מחקי</a>
              </div>
            </div>
          <?php else: ?>
            <form action="save_glucose_comment.php" method="POST">
              <div class="mb-3">
                <label class="form-label">🗨️ כתבי הערה:</label>
          ...
          <?php if (!empty($reading['doctor_note'])): ?>
            <div class="alert alert-warning persistent mt-3">
...
<script>
  document.querySelectorAll('.alert:not(.persistent)').forEach(alertBox => {
    setTimeout(() => {
