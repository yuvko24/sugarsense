...
// Fetch existing dietitian note
$existingNote = '';
$stmt = $conn->prepare("SELECT dietitian_note FROM glucose_readings WHERE id = ?");
$stmt->bind_param("i", $readingId);
$stmt->execute();
$result = $stmt->get_result();
if ($row = $result->fetch_assoc()) {
    $existingNote = $row['dietitian_note'];
}
$stmt->close();

// Fetch patient id
$stmt = $conn->prepare("SELECT patient_id FROM glucose_readings WHERE id = ?");
$stmt->bind_param("i", $readingId);
$stmt->execute();
$result = $stmt->get_result();
if ($row = $result->fetch_assoc()) {
...
// Fetch patient name and picture
$patientName = "לא ידוע";
...
<head>
  ...
  <title>✏️ עריכת הערת ניטור סוכר</title>
...
<main class="container py-5 flex-grow-1">
  <div class="position-relative text-center mb-4">
    <a href="glucose_history_dietitian.php" class="btn btn-outline-secondary btn-sm position-absolute top-0 start-0 mt-1 ms-2">🔙 חזרה</a>
    <h2 class="mb-0">✏️ עריכת הערת ניטור סוכר</h2>
  </div>

  <form action="save_glucose_comment.php" method="POST" class="mx-auto" style="max-width: 600px;">
    <div class="mb-3">
